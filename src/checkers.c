...
int color_checker(char *new_str, game_p *game, int side)
{
    ...
    return(create_rgb(r, g, b, side, game));
}
...
int check_pos_valid(size_t y_pos, size_t x_pos, game_p *game)
{
    ...
    if (x_pos == 0 || x_pos == ft_strlen(matrix[y_pos]) - 1 || y_pos == 0 || y_pos == y_size - 1
                    || ft_strlen(matrix[y_pos - 1]) < x_pos || ft_strlen(matrix[y_pos + 1]) < x_pos )
