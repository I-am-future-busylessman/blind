...
{
    if (game->map.north_texture == NULL || game->map.south_texture == NULL 
        || game->map.west_texture == NULL || game->map.east_texture == NULL
        || game->map.sprite_texture == NULL || game->map.ceiling < 0
        || game->map.floor < 0 || game->window.height < 0 || game->window.width < 0)
...
int     line_identify(char *new_str, game_p *game, int status)
{
    ...
    if (new_str[skipped] == '1')
    {
        if (config_check(game))
            return(game->error);
        return(map_collector(new_str, game, status));
    }
        ...
        return(color_checker(new_str + skipped, game, 1));
    else if (new_str[skipped] == 'C')
        return(color_checker(new_str + skipped, game, 2));
    else
        return(0);
}
...
int     start_parser(game_p *game)
{
    ...
    while ((game->error = get_next_line(file_descriptor, &new_str)))
    {
        ...
        if ((line_identify(new_str, game, 0)) < 0)
            return (game->error);
    }
    game->error = line_identify(new_str, game, 1);
